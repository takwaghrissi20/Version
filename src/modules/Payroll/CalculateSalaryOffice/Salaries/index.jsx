...
const Table = () => {
  ...
  const fetchAllListEmployee = async () => {
    try {
      ...
      const url = `${endPoint}/api/v1/emp/getEmByTypeWithoutPage?type=office&token=${token}`;
  ...
  const filterEmployeesByProjectAndMonth = () => {
    ...
    return allemployee.filter(emp => {
      ...
      const hasPointagesInMonth = emp.officepointages.some(pointage => pointage.month === selectedMonth && pointage.pointage !== null);
