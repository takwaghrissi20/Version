...
const AllRequestPaymentTab = ({ user }) => {
  ...
  const getBODStatus = (notificationValue) => {
  ...
  const getBODStatusClass = (notificationValue) => {
    if (notificationValue === 1) {
      return 'pending-bod-status';  
    }
    return 'unknown-bod-status';  
  };
  //TabHeight
  ...
  const AllRequestPayment = async () => {
    try {
      ...
      const filterData = data.filter(p => p.notif === 0 ||p.notif === 5 ||
        p.notif === 1
  ...
  const columns = [
    ...
    
    {
      ...
      key: 'notif',
      render: (text, record) => {
        const status = getBODStatus(record.notif);
        const statusClass = getBODStatusClass(record.notif);
        return <span className={statusClass}>{status}</span>;
      },
    },
    // {
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <AppCard
          ...
          <Space direction='vertical' style={{ width: '100%', marginBottom: "8rem" }}>
            <div>
              <h4 style={{ padding: "1rem" }}> Pending Request Payment</h4>
              <StyledOrderTable
                hoverColor
...
export default AllRequestPaymentTab;
