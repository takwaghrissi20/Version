...
const AllRequestPaymentApprovedTab = ({ user }) => {
  ...
  const getBODStatus = (record) => {
    if (record.notif === 3 && record.approvedByBod1 === "true") {
      return <p>Approved by BOD1</p>;
    }
    if (record.notif === 3 && record.approvedByBod2 === "true") {
      return <p>Approved by BOD2</p>;
    }
    return <p></p>; 
  };
  //To get classs
  ...
  const AllRequestPayment = async () => {
    try {
      ...
      const filterData = data.filter(p => 
        p.notif === 1 || 
        (p.notif === 3 && p.approvedByBod1 === "true") || 
        (p.notif === 3 && p.approvedByBod2 === "true")
      );
...
export default AllRequestPaymentApprovedTab;
