...
const SiteSalary = () => {
  ...
  const [selectprojet, setSelectprojet] = useState('');
  ...
  const GetPositions = async () => {
    ...
    try {
      ...
      const responseConstruction = await fetch(`https://dev-gateway.gets-company.com/api/v1/rateConst/list`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      const dataConstructuction=await responseConstruction.json();
      const Position = data.map(p => p.description)
      const PositionConstruction = dataConstructuction.map(p => p.description)
      const allPositions = [...Position, ...PositionConstruction]; 
  ...
  const openNotification = () => {
    notification.open({
      message: 'Success',
      description: 'Success Add Site-SALARY INCREASE _POSITION CHANGE ',
      style: {
  ...
  const openNotificationError = () => {
    notification.open({
      message: 'Error',
      description: 'Error Add Site-SALARY INCREASE _POSITION CHANGE ',
      style: {
        backgroundColor: 'red',
    ...
    try {
      const response = await fetch(`https://dev-gateway.gets-company.com/api/v1/position/save?token=${token}`, {
        ...
        body: JSON.stringify({
          ...
          name:nameEmp,
          getsId: getsEmployeeId,
          actualPositions:positionIncrease,
    ...
    <>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <div>
            <Typography.Title level={4}>Site-SALARY INCREASE _POSITION CHANGE</Typography.Title>
          </div>
        </div>
              ...
              <AppRowContainer>
                ...
                <Col xs={24} md={12}>
                  <Form.Item label='Assignement Project' name='Assignement Project'
                    ...
                    rules={[
                      { required: true, message: 'Please input your Assignement Project!' }]}>
                    <Select
                      placeholder='Select Assignement Project'
                      value={selectprojet}
